  ...
  return (
    ...
    >
      ...
      <header className="relative z-10 flex shrink-0 flex-wrap items-start justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2 font-mono text-[11px] font-semibold uppercase tracking-wider text-white/65">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/12 bg-white/[0.06] px-2 py-0.5 text-white/75">
            <span
              aria-hidden
          ...
          <span className="rounded-full border border-white/10 bg-white/[0.05] px-2 py-0.5 text-white/70">
            {categoryLabel}
          </span>
        </div>
      ...
      <div className="relative z-10 mt-5 grid min-h-0 flex-1 grid-cols-1 gap-6 overflow-hidden lg:grid-cols-[minmax(0,1.14fr)_minmax(0,0.86fr)] lg:gap-8">
        ...
        <div className="flex min-h-0 flex-col gap-4 overflow-y-auto overscroll-y-contain pr-0.5 lg:pr-1">
          <h2
            ...
            className="text-balance text-2xl font-bold tracking-tight text-white md:text-3xl md:leading-tight"
          >
            {project.title}
          </h2>
          <p className="text-pretty text-base font-semibold leading-relaxed text-white/85">
            {project.summary}
          </p>
          <p className="text-pretty text-sm font-medium leading-relaxed text-white/75 md:text-base">
            {project.description}
          </p>
          ...
          {detailText ? (
            ...
            >
              ...
              <p className="mt-2 max-h-[min(22vh,11rem)] overflow-y-auto overscroll-y-contain text-pretty text-sm font-medium leading-relaxed text-white/75">
                {detailText}
              </p>
            </section>
          ) : null}
          ...
          <dl className="grid gap-2 text-sm">
            <div className="flex flex-wrap gap-x-2">
              <dt className="font-mono text-[10px] font-semibold uppercase tracking-wider text-white/45">
                {t("roleLabel")}
              </dt>
              <dd className="font-medium text-white/85">{project.role}</dd>
            </div>
            {project.client ? (
              <div className="flex flex-wrap gap-x-2">
                <dt className="font-mono text-[10px] font-semibold uppercase tracking-wider text-white/45">
                ...
                <dd className="font-medium text-white/80">{project.client}</dd>
              </div>
            ) : null}
          </dl>
          ...
          <div>
            ...
            <ul className="mt-2 flex list-none flex-wrap gap-1.5 p-0">
              {project.stack.map((item) => (
                <li
                  key={item}
                  className="rounded-full border border-white/14 bg-white/[0.08] px-2.5 py-0.5 text-[11px] font-semibold text-white/88 transition-[border-color,background-color,transform] duration-200 ease-out hover:border-primary/40 hover:bg-white/[0.12] hover:text-white motion-reduce:hover:scale-100 sm:hover:scale-[1.03]"
                >
                  {item}
                </li>
              ))}
                  ...
                  return (
                    <li key={`${link.type}-${link.url}`}>
                      <a
                        ...
                        className="group/link inline-flex items-center gap-1.5 rounded-lg border border-white/14 bg-white/[0.07] px-3 py-2 text-xs font-semibold text-white/90 transition-[transform,border-color,background-color] duration-200 ease-out hover:-translate-y-0.5 hover:border-primary/50 hover:bg-white/[0.12] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 motion-reduce:hover:translate-y-0"
                      >
